...
# ============================
SESSION = environ.get('SESSION', 'LeoX')   # Session name for the bot
API_ID = int(environ.get('API_ID', '27549502')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '35892287036e27af8e9298f03483fdd4')  # API Hash from my.telegram.org
BOT_TOKEN = environ.get('BOT_TOKEN', "7735629958:AAEGyrV4hU5_oY4PVei2HuPCDsmF8oOWWPA")    # Bot token from @BotFather
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7844061005').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002805106938 -1003074947898').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002623246805'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002757225573'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1002623246805'))  # Premium logs channel id
...
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003295557812')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-100')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/+PI-oUjBzaTMzYjQ1')  # Support group link (make sure bot is admin)
...
auth_channels     = environ.get("AUTH_CHANNELS", "-1002595154363 -1002553652611")# Channels for force sub (make sure bot is admin)

# ============================
# Payment Configuration
# ============================
QR_CODE = environ.get('QR_CODE', 'https://i.ibb.co/DTyjSVz/Screenshot-2025-12-09-19-10-36-106-com-google-android-apps-nbu-paisa-user-edit.jpg')    # QR code image for payments
OWNER_UPI_ID = environ.get('OWNER_UPI_ID', 'yuxe@ybl')    # Owner UPI ID for payments
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Shakthy:Shakthy10@cluster0.j2itrtc.mongodb.net/?appName=Cluster0")  # MongoDB URI for the database
...
DATABASE_URI2 = environ.get('DATABASE_URI2', "mongodb+srv://shakthy:Shakthy10@cluster01.yqdp8vd.mongodb.net/?appName=Cluster01")  # MongoDB URI for the second database (if MULTIPLE_DB is True)
# ============================
# Movie Notification & Update Settings
# ============================
MOVIE_UPDATE_NOTIFICATION = bool(environ.get('MOVIE_UPDATE_NOTIFICATION', False))  # Notification On (True) / Off (False)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002553652611'))  # Notification of sent to your channel
...
ABOVE_PREVIEW = bool(environ.get('ABOVE_PREVIEW', False)) # Shows link preview above the text in notification msg if True else below the msg
TMDB_API_KEY = environ.get('TMDB_API_KEY', '7eac9f64df842916169890a469f545c7') # preffer to use your own tmdb API Key get it from https://www.themoviedb.org/settings/api
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/Get1TamilMoviez') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/leodassx') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/Get1TamilMV') # Update channel link for the bot
...
LANGUAGES = {"ᴛᴀᴍɪʟ":"tam","ᴍᴀʟᴀʏᴀʟᴀᴍ":"mal","ᴛᴇʟᴜɢᴜ":"tel","ᴋᴀɴɴᴀᴅᴀ":"kan","ᴇɴɢʟɪsʜ":"eng","ʜɪɴᴅɪ":"hin"}
...
URL = "https://huge-christean-shakthyoffl-a30acc50.koyeb.app/".format(FQDN) if ON_HEROKU or NO_PORT else "https://huge-christean-shakthyoffl-a30acc50.koyeb.app/".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://huge-christean-shakthyoffl-a30acc50.koyeb.app/".format(FQDN)
else:
    URL = "http://huge-christean-shakthyoffl-a30acc50.koyeb.app/".format(FQDN)
